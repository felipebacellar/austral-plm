...
export default function FichaModal({ row, onClose }: Props) {
  ...
  const [imgFrente, setImgFrente] = useState<string|null>(row.ficha?.imagem_estampa_frente || null);
  const [imgCostas, setImgCostas] = useState<string|null>(row.ficha?.imagem_estampa_costas || null);
  ...
  const handleUpload = async (file: File, field: string, setter: (url:string)=>void) => {
    setUploading(true);
    const url = await uploadImage(file, `${row.ref}/${field}`);
    if (url) {
      setter(url);
      // Save to database if ficha exists
      if (row.ficha?.id) {
        await saveFichaImagem(row.ficha.id, field, url);
      }
    }
    setUploading(false);
  };

  const handleImageChange = (e: any, field: string, setter: (url:string)=>void) => {
    const file = e.target.files?.[0];
    if (file) handleUpload(file, field, setter);
  };
          ...
          <div className="px-6 pb-8">
            ...
            <div className="border border-t-0 border-gray-200 rounded-b-xl overflow-hidden mb-4">
              <div className="grid grid-cols-2 text-[13px]">
                ...
                <F l="Data atualiz." v={new Date().toLocaleDateString("pt-BR")}/><F l="Operação" v={row.operacao}/>
                <F l="Estilista" v={row.estilista}/><F l="Fornecedor" v={row.fornecedor}/>
              </div>
              <div className="grid grid-cols-3 text-[13px]">
                <F l="Drop" v={row.drop}/><F l="Grade" v={row.grade}/><div/>
              </div>
            ...
            {/* Desenho técnico */}
            <ImageUpload image={image} inputRef={fileRef} label="Desenho técnico" sublabel="Clique para enviar JPEG ou PNG" aspect="aspect-[16/9] max-h-[380px]"
              onChange={(e:any)=>handleImageChange(e,"imagem_url",setImage)} />
            <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e)=>handleImageChange(e,"imagem_url",setImage)}/>
            ...
            <div className="border border-gray-200 rounded-xl overflow-hidden mb-4 overflow-x-auto">
              <table className="w-full text-[13px] border-collapse">
                <thead><tr className="bg-gray-50 text-[10px] font-bold uppercase tracking-wider text-gray-400">
                  <th className="text-left px-3 py-2">Artigo</th><th className="text-left px-2 py-2">Fornec.</th><th className="px-2 py-2">Preço/cons.</th><th className="px-2 py-2">Código</th><th className="text-left px-2 py-2">Localiz.</th>
                ...
                <tbody>
                  {f.tecidos.length>0?f.tecidos.map((t:any,i:number)=>(
                    <tr key={i} className="border-b border-gray-100">
                      ...
                      <td className="px-2 py-2 text-gray-500 text-xs text-center">{t.codigo||"—"}</td>
                      ...
                      {Array.from({length:Math.max(0,4-(t.cores||[]).length)}).map((_,j)=><td key={`e${j}`} className="px-2 py-2 text-center text-gray-300">—</td>)}
                    </tr>
                  )):<tr><td colSpan={9} className="py-5 text-center text-gray-400">Nenhum tecido</td></tr>}
                </tbody>
              </table>
            </div>
            ...
            {f.tecidos.length>0&&f.tecidos[0]?.cores?.length>0&&(
              <div className="border border-gray-200 rounded-xl overflow-hidden mb-4">
                <div className="flex items-center px-3 py-2.5">
                  <span className="font-bold text-[13px] text-gray-700 flex-1">Quantidade de peças de mostruário:</span>
                  {f.tecidos[0].cores.slice(0,4).map((c:string,i:number)=>(
                    <span key={i} className="w-24 text-center font-bold text-lg tabular-nums">{f.qtdMostruario?.[c]||"—"}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="border-t-4 border-[#1e3a5f] pt-4 mt-6">
              ...
              <div className="border border-gray-200 rounded-xl overflow-hidden mb-4 overflow-x-auto">
                <table className="w-full text-[13px] border-collapse">
                  <thead><tr className="bg-gray-50 text-[10px] font-bold uppercase tracking-wider text-gray-400">
                    <th className="text-left px-3 py-2">Matéria prima</th><th className="text-left px-2 py-2">Código</th><th className="text-center px-2 py-2 w-12">Qtde</th><th className="text-right px-2 py-2 w-16">Valor</th><th className="text-left px-2 py-2">Localização</th>
                    <th className="text-center px-2 py-2 w-24">Var 01</th><th className="text-center px-2 py-2 w-24">Var 02</th><th className="text-center px-2 py-2 w-24">Var 03</th><th className="text-center px-2 py-2 w-24">Var 04</th>
                  </tr></thead>
                  <tbody>
                    {f.aviamentos.length>0?(<>
                      {f.aviamentos.map((a:any,i:number)=>(
                        <tr key={i} className="border-b border-gray-100 hover:bg-gray-50/50">
                          ...
                          <td className="px-2 py-2 text-[12px] text-gray-500">{a.local}</td>
                      ...
                      <tr className="border-t border-gray-300 font-bold"><td colSpan={3} className="px-3 py-2">Total</td><td className="px-2 py-2 text-right tabular-nums">R$ {avTotal.toFixed(2)}</td><td colSpan={5}/></tr>
                    </>):<tr><td colSpan={9} className="py-5 text-center text-gray-400">Nenhum aviamento</td></tr>}
                  </tbody>
                </table>
              </div>
              ...
              <div className="border border-gray-200 rounded-xl overflow-hidden mb-4">
                ...
                <table className="w-full text-[13px] border-collapse">
                  ...
                  <tbody>{(f.pilotagem||[]).map((p:any,i:number)=>(
                    <tr key={i} className="border-b border-gray-100">
                      <td className="px-3 py-2 font-medium">{p.num}</td><td className="px-2 py-2 text-gray-500">{p.lacre||"—"}</td><td className="px-2 py-2 text-gray-500">{p.envio||"—"}</td><td className="px-2 py-2 text-gray-500">{p.receb||"—"}</td><td className="px-2 py-2 text-gray-500">{p.prova||"—"}</td>
                      <td className="px-2 py-2">{p.status?<span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${SB[p.status]||"bg-gray-100 text-gray-500"}`}>{p.status}</span>:"—"}</td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
        ...
        {tab === "estamparia" && (
          <div className="px-6 pb-8">
            ...
            <div className="border border-t-0 border-gray-200 rounded-b-xl overflow-hidden mb-4">
              <div className="grid grid-cols-2 text-[13px]">
                <F l="Referência" v={row.ref}/><F l="Descrição" v={row.desc}/>
                <F l="Operação" v={row.operacao}/><F l="Fornecedor" v={row.fornecedor}/>
                <F l="Estilista" v={row.estilista}/><F l="Grade" v={row.grade}/>
              </div>
            </div>

            {/* Image areas frente/costas com upload */}
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <ImageUpload image={imgFrente} inputRef={fileFrenteRef} label="Frente" aspect="aspect-[4/3]"
                  onChange={(e:any)=>handleImageChange(e,"imagem_estampa_frente",setImgFrente)}/>
                <input ref={fileFrenteRef} type="file" accept="image/*" className="hidden" onChange={(e)=>handleImageChange(e,"imagem_estampa_frente",setImgFrente)}/>
              </div>
              <div>
                <ImageUpload image={imgCostas} inputRef={fileCostasRef} label="Costas" aspect="aspect-[4/3]"
                  onChange={(e:any)=>handleImageChange(e,"imagem_estampa_costas",setImgCostas)}/>
                <input ref={fileCostasRef} type="file" accept="image/*" className="hidden" onChange={(e)=>handleImageChange(e,"imagem_estampa_costas",setImgCostas)}/>
              </div>
            </div>
            ...
            <div className="border border-gray-200 rounded-xl overflow-hidden mb-4">
              <div className="bg-gray-50 border-b border-gray-200">
                <div className="grid grid-cols-[60px_2fr_1fr_1fr_1fr_1fr] text-[10px] font-bold uppercase tracking-wider text-gray-400 px-3 py-2">
                  <span>#</span><span>Técnica de estamparia</span><span className="text-center">Var 01</span><span className="text-center">Var 02</span><span className="text-center">Var 03</span><span className="text-center">Var 04</span>
                </div>
              </div>
              {(f.estamparia?.tecnicas||[]).length>0?(f.estamparia.tecnicas.map((t:any,i:number)=>(
                <div key={i} className="grid grid-cols-[60px_2fr_1fr_1fr_1fr_1fr] text-[13px] px-3 py-2 border-b border-gray-100">
                  <span className="text-gray-400">{t.num||i+1}</span><span className="font-medium">{t.tecnica||"—"}</span>
                  <span className="text-center">{t.var01||"—"}</span><span className="text-center">{t.var02||"—"}</span><span className="text-center">{t.var03||"—"}</span><span className="text-center">{t.var04||"—"}</span>
                </div>
              ))):<div className="px-3 py-5 text-sm text-gray-400 text-center">Nenhuma técnica cadastrada</div>}
            </div>
...
function F({l,v}:{l:string;v:any}) {
  return <div className="flex items-baseline gap-2 px-3 py-1.5 border-b border-r border-gray-100"><span className="text-[11px] text-gray-400 whitespace-nowrap">{l}:</span><span className="text-[13px] font-medium text-gray-900">{v||"—"}</span></div>;
}

function ImageUpload({image,inputRef,label,sublabel,aspect,onChange}:{image:string|null;inputRef:any;label:string;sublabel?:string;aspect:string;onChange:(e:any)=>void}) {
  return (
    <div className={`border border-gray-200 rounded-xl overflow-hidden mb-4 bg-gray-50 cursor-pointer hover:border-blue-300 transition-colors ${aspect}`}
