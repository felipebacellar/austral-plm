...
export default function FichaModal({ row, onClose }: Props) {
  ...
  const handleImg = (e: any, field: string, setter: (url:string)=>void) => { const file=e.target.files?.[0]; if(file) handleUpload(file,field,setter); };
      ...
      <div className="bg-white rounded-2xl w-full max-w-[960px] shadow-2xl overflow-hidden" onClick={e=>e.stopPropagation()}>
        ...
        {tab === "ficha" && (
          <div className="px-6 pb-8">
            ...
            <ImgUp image={image} inputRef={fileRef} label="Desenho técnico" sub="Clique para enviar JPEG ou PNG" aspect="aspect-[16/9] max-h-[380px]" onChange={e=>handleImg(e,"imagem_url",setImage)}/>
            ...
            <div className="border-t-4 border-[#1e3a5f] pt-4 mt-6">
              ...
              <div className="border border-gray-200 rounded-xl overflow-hidden mb-4 overflow-x-auto">
                <table className="w-full text-[13px] border-collapse">
                  ...
                  <tbody>{f.aviamentos.length>0?(<>
                    {f.aviamentos.map((a:any,i:number)=>(
                      <tr key={i} className="border-b border-gray-100 hover:bg-gray-50/50">
                        <td className="px-3 py-2 font-medium">{a.item}</td><td className="px-2 py-2 font-mono text-[11px] text-gray-500">{a.cod}</td>
                        <td className="px-2 py-2 text-center tabular-nums">{a.qtd}</td><td className="px-2 py-2 text-right tabular-nums">{a.valor.toFixed(2)}</td>
                        <td className="px-2 py-2 text-[12px] text-gray-500">{a.local}</td>
                        <td className="px-2 py-2 text-center text-[12px]">{a.var01||"—"}</td><td className="px-2 py-2 text-center text-[12px]">{a.var02||"—"}</td>
                        <td className="px-2 py-2 text-center text-[12px]">{a.var03||"—"}</td><td className="px-2 py-2 text-center text-[12px]">{a.var04||"—"}</td>
                      </tr>
                    ))}
                  ...
                  </>):<tr><td colSpan={9} className="py-5 text-center text-gray-400">Nenhum aviamento</td></tr>}</tbody>
                </table>
              </div>
        ...
        {tab === "estamparia" && (
          <div className="px-6 pb-8">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <ImgUp image={imgFrente} inputRef={fileFrenteRef} label="Frente" aspect="aspect-[4/3]" onChange={e=>handleImg(e,"imagem_estampa_frente",setImgFrente)}/>
                <input ref={fileFrenteRef} type="file" accept="image/*" className="hidden" onChange={e=>handleImg(e,"imagem_estampa_frente",setImgFrente)}/>
              </div>
              <div>
                <ImgUp image={imgCostas} inputRef={fileCostasRef} label="Costas" aspect="aspect-[4/3]" onChange={e=>handleImg(e,"imagem_estampa_costas",setImgCostas)}/>
                <input ref={fileCostasRef} type="file" accept="image/*" className="hidden" onChange={e=>handleImg(e,"imagem_estampa_costas",setImgCostas)}/>
              </div>
            </div>
            <div className="border border-gray-200 rounded-xl overflow-hidden mb-4">
              <div className="bg-gray-50 border-b border-gray-200">
                <div className="grid grid-cols-[60px_2fr_1fr_1fr_1fr_1fr] text-[10px] font-bold uppercase tracking-wider text-gray-400 px-3 py-2">
                  <span>#</span><span>Técnica</span><span className="text-center">Var 01</span><span className="text-center">Var 02</span><span className="text-center">Var 03</span><span className="text-center">Var 04</span>
                </div>
              </div>
...
function ImgUp({image,inputRef,label,sub,aspect,onChange}:{image:string|null;inputRef:any;label:string;sub?:string;aspect:string;onChange:(e:any)=>void}) {
  return (
    <div className={`border border-gray-200 rounded-xl overflow-hidden mb-4 bg-gray-50 cursor-pointer hover:border-blue-300 transition-colors ${aspect}`} onClick={()=>inputRef.current?.click()}>
      <div className="w-full h-full flex items-center justify-center">
        {image ? <img src={image} alt={label} className="w-full h-full object-contain p-2"/> : (
