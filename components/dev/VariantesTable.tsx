...
import { SAMPLE_ROWS, SAMPLE_VARIANTES } from "@/lib/sample-data";
...
const SP: Record<string,string> = {
  "MOSTRUÁRIO LIBERADO": "bg-emerald-50 text-emerald-700",
  "PRODUÇÃO LIBERADA": "bg-blue-50 text-blue-700",
  "DESENVOLVIMENTO": "bg-amber-50 text-amber-700",
...
export default function VariantesTable({ onOpenFicha }: Props) {
  ...
  // Build variant rows: one row per product x color
  const variantRows = useMemo(() => {
    const rows: any[] = [];
    SAMPLE_ROWS.forEach((prod: any) => {
      const cores = SAMPLE_VARIANTES[prod.ref] || [];
      if (cores.length === 0) {
        // Product with no variants — show one row with no color
    ...
    <div>
      <div className="flex gap-2 mb-5 flex-wrap items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            type="text"
            placeholder="Buscar referência, cor, tecido..."
            value={query}
      ...
      <div className="flex items-baseline gap-3 mb-4 pl-1">
        ...
        <span className="text-xs text-gray-400 ml-auto">Gerado a partir das cores cadastradas na ficha técnica</span>
      </div>
      ...
      <div className="overflow-x-auto rounded-2xl border border-gray-200 bg-white">
        <table className="plm-table" style={{ width: "max-content", minWidth: "100%" }}>
          ...
          <tbody>
                ...
                {VCOLS.map(c => (
                  <td key={c.key} style={{ width: c.w, minWidth: c.w }}>
                    {c.key === "_ficha" ? (
                      ...
                      <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full whitespace-nowrap ${SP[row.status]}`}>
                        {row.status}
                      </span>
                    ) : c.key === "cor" && row.cor !== "—" ? (
                      <span className="inline-flex items-center gap-1.5 bg-gray-100 rounded-lg px-2.5 py-1 text-[13px] font-medium">
                        {row.cor}
                      </span>
                    ) : (
                      <span className={`text-[13px] px-2 py-1 ${row[c.key] ? "text-gray-900" : "text-gray-300"}`}>
                        {row[c.key] || "—"}
                      </span>
                    )}
                  </td>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={VCOLS.length} className="py-12 text-center text-gray-400 text-sm">
