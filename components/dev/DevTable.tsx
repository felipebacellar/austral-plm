...
export default function DevTable({ rows, setRows, onOpenFicha }: Props) {
  ...
  const upd = async (id:number, k:string, v:string|number) => {
    ...
    // Also save forn_tecido if tecido changed
  ...
  const add = async () => {
    ...
    if(result) {
      setRows((p:any) => [...p, { ...blank, id: result.id }]);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="flex items-baseline gap-3 mb-4"><span className="text-[28px] font-bold tabnum tracking-[-0.03em]">{filtered.length}</span><span className="text-[14px] text-[var(--label-secondary)]">SKU{filtered.length!==1&&"s"}</span>{ac>0&&<span className="text-[12px] text-[var(--label-tertiary)]">de {rows.length}</span>}<span className="text-[11px] text-[var(--label-quaternary)] ml-auto italic">duplo-clique para editar · salva automaticamente</span></div>
