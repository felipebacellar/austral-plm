    ...
    <div className="flex gap-6 min-h-[340px]">
      <div className="w-48 flex-shrink-0">
        ...
        <nav className="flex flex-col gap-px">
          {TABS.map(t=>{const cnt=getCount(t.k);const on=mode===t.k;return(
            <button key={t.k} onClick={()=>{setActive(t.k);setSearch("");}}
              className={`flex justify-between items-center px-3 py-2 rounded-lg text-sm text-left transition-all ${on?"font-semibold bg-blue-50 text-[#007AFF]":"text-gray-900 hover:bg-gray-50"}`}>
              <span>{t.l}</span>
              <span className={`text-xs tabular-nums ${on?"text-blue-400":"text-gray-400 bg-gray-100 px-1.5 rounded"}`}>{cnt}</span>
            </button>
          );})}
        </nav>
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        {mode==="cor"&&(<>
          ...
          <div className="border border-gray-200 rounded-xl overflow-hidden"><table className="plm-table"><thead><tr><th className="w-24">Código</th><th>Nome</th><th className="w-10"></th></tr></thead>
            <tbody>{cad.cor.map((c:any)=>(<tr key={c.cod}><td className="font-mono font-bold text-gray-500">{c.cod}</td><td className="font-medium">{c.nome}</td><td className="text-center"><button onClick={()=>remCor(c.cod)} className="text-gray-400 hover:text-red-500">×</button></td></tr>))}</tbody>
          </table></div>
        </>)}
        ...
        {mode==="aviamento"&&(<>
          <div className="flex gap-2 mb-3">
            ...
            <input className={`flex-1 ${inp}`} value={avNome} onChange={e=>setAvNome(e.target.value)} placeholder="Nome do aviamento"/>
        ...
        {mode==="tecido"&&(<>
          <div className="flex gap-1.5 mb-5 flex-wrap">
            ...
            <input className={`flex-1 min-w-[100px] ${inp}`} value={tf} onChange={e=>setTf(e.target.value)} placeholder="Fornecedor"/>
            <input className={`flex-1 min-w-[100px] ${inp}`} value={tc} onChange={e=>setTc(e.target.value)} placeholder="Composição"/>
            <input className={`w-20 ${inp}`} value={tp} onChange={e=>setTp(e.target.value)} placeholder="Preço"/>
            <button onClick={addTecido} className={btn}>Adicionar</button>
          </div>
